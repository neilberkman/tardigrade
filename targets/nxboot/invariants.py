...
def check_nxboot_roles_distinct(result, **_):
    ...
    raise InvariantViolation(
        invariant_name="nxboot_roles_distinct",
        description="nxboot update and recovery roles collapsed onto the same slot.",
        result=result,
        details={"update_slot": update_slot, "recovery_slot": recovery_slot},
    )


def check_nxboot_confirmed_has_recovery(result, **_):
    root = _semantic_root(result)
    roles = _roles(root)
    ...
    if roles.get("recovery_valid") and roles.get("recovery_present"):
        return
    raise InvariantViolation(
        invariant_name="nxboot_confirmed_has_recovery",
        description=(
...
def check_nxboot_duplicate_update_consumed(result, **_):
    root = _semantic_root(result)
    roles = _roles(root)
    flags = _flags(root)
    ...
    if roles.get("next_boot") == "none":
        return
    raise InvariantViolation(
        invariant_name="nxboot_duplicate_update_consumed",
        description=(
